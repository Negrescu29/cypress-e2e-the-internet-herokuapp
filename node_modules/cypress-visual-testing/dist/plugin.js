...
function compareSnapshotsPlugin(args) {
  return new Promise((resolve, reject) => {
    const options = {
      actualImage: path.join(SNAPSHOT_DIRECTORY, 'actual', args.specDirectory, `${args.fileName}.png`),
      expectedImage: path.join(SNAPSHOT_DIRECTORY, 'base', args.specDirectory, `${args.fileName}.png`),
      diffImage: path.join(SNAPSHOT_DIRECTORY, 'diff', args.specDirectory, `${args.fileName}.png`)
    };
    ...
    fs.createReadStream(options.actualImage).pipe(new PNG()).on('parsed', function () {
      ...
      if (!fs.existsSync(options.expectedImage)) {
        if (options.strict) {
          drawBlackImage(imgActual.width, imgActual.height, options.expectedImage);
        } else {
      ...
      fs.createReadStream(options.expectedImage).pipe(new PNG()).on('parsed', function () {
        ...
        if (checksum(imgActual.data, 'sha1') !== checksum(imgExpected.data, 'sha1')) {
          ...
          if (imgActual.width === imgExpected.width && imgActual.height === imgExpected.height) {
            mismatchedPixels = pixelmatch(imgActual.data, imgExpected.data, diff.data, imgActual.width, imgActual.height, { threshold: 0.1 });
